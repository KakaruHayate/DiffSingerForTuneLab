...
public sealed class VoiceRegistry
{
    ...
    // —— 跨 voicebank 同模型候选枚举（供引擎 fingerprint + CompatibleVoices 构建）——
...
// 跨 voicebank 候选：来自另一个 voicebank 的同模型 voice（非声码器 ONNX 相同），可加入当前 voice 的说话人混合。
...
// 一个 part 的解析上下文：当前 voicebank 能力集 + voice 语境 + 注册表（取 model/version 下拉选项）+ 同模型外部候选。
public sealed record PartContext(
    VoicebankConfig Config, ResolvedVoice Resolved, VoiceRegistry Registry, string VoiceId)
{
    // 跨 voicebank 混音候选（非声码器 ONNX 与当前 voicebank 相同、来自其他已安装 voicebank 的 voices）。
